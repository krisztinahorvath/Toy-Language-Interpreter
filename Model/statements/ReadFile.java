    ...
    public MyIDict<String, IType> typeCheck(MyIDict<String, IType> typeEnv) throws StmtExeException, ExprEvalException, ADTException {
        if (expr.typeCheck(typeEnv).equals(new StringType()))
            if (typeEnv.lookUp(varName).equals(new IntType()))
                ...
                throw new StmtExeException("ERROR: ReadFile requires an int as its variable parameter.");
        else
            throw new StmtExeException("ERROR: ReadFile requires a string as es expression parameter.");
    }
